...
//		$this->getStatement()->delete('sc_cache',"name='ModuleManager::module'");
    ...
    function actRedirect() {
        $rs = DB::select("SELECT * FROM sc_enum WHERE field_name='redirect' ORDER BY position");
        $data = array('rs' => $rs);
        $this->display($data, dirname(__FILE__) . '/redirect.tpl.php');
    }

    function actSaveRedirect() {
        global $post;

        $field_value = $post->getArray('field_value');
        $value_desc = $post->getArray('value_desc');
        $position = $post->getArray('position');
        DB::delete('sc_enum', "field_name='redirect'");
        $i = 0;
        foreach ($field_value as $k => $v) {
            ...
            if (trim($field_value[$k]) != '') {
                ...
                DB::insert('sc_enum', $d, null);
            }
        }
        if ($import = trim($post->get('import'))) {
            $lines = explode("\n", $import);
            foreach ($lines as $line) {
                if ($line = trim($line)) {
                    $row = explode(';', $line);
                    $rs = DB::select("SELECT * FROM sc_enum WHERE field_value='" . SQL::slashes($row[0]) . "' AND field_name='redirect'");
                    if ($rs->next()) {
                        $d = array( 'value_desc' => $row[1],  );
                        DB::update('sc_enum', $d, "field_value='" . SQL::slashes($row[0]) . "' AND field_name='redirect'");
                    } else {
                        $d = array(
                            'field_value' => $row[0],
                            'value_desc' => $row[1],
